...
const emptyTitle = computed(() => search.value ? 'No results' : 'No posts yet')
const emptyDesc  = computed(() => search.value ? 'No posts match "' + search.value + '"' : 'Check back soon.')
</script>

<template>
  <div class="mx-auto max-w-6xl px-6 py-16">
    <div class="mb-10">
      <p class="text-xs font-semibold uppercase tracking-widest text-[var(--color-brand-400)]">Writing</p>
      <h1 class="mt-1 font-display text-4xl font-bold text-[var(--text-primary)]">Blog</h1>
      <p class="mt-3 max-w-lg text-[var(--text-secondary)]">
        Thoughts on software, architecture, and things I've learned.
      </p>
    </div>
    ...
    <div v-else class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
      <BlogPostCard v-for="post in filtered" :key="post.id" :post="post" />
    </div>
  </div>
</template>
